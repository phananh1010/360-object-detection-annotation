...
class VideoRecorder(ImageRecorder):
    def __init__(self, video_in, video_out, x0=0., y0=0., view_angle=65.5):
        ...
        sphereW = video.get(cv2.CAP_PROP_FRAME_WIDTH)
        sphereH = video.get(cv2.CAP_PROP_FRAME_HEIGHT)
        self.fps = video.get(cv2.CAP_PROP_FPS)
        print "Super class: {}".format(super)
        print "sphereW: {}  sphereH: {}".format(sphereW, sphereH)
    ...
    def _capture_next(self):
        ...
        if not ret:
            ...
            self._current += 1
            print "_x: {}  _y: {}".format(self._x, self._y)
